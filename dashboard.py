...
def financial_summary(symbol: str):
    ...
    Tuliskan ringkasan eksekutif berdasarkan data keuangan berikut ini:
...
def revenue_trend(symbol: str, financials: pd.DataFrame):
    ...
    Instruksi:
    - Sumbu X adalah tanggal (date)
    - Sumbu Y adalah pendapatan (revenue)
    - Beri judul 'Tren Pendapatan Perusahaan yang dipilih
...
def trend_analysis(financials: pd.DataFrame):
    ...
    Berdasarkan data pendapatan kuartalan berikut (dalam miliar rupiah):
    {data}
    Analisis tren utama yang muncul dari data tersebut, termasuk pola pertumbuhan, fluktuasi musiman, dan anomali yang signifikan.
    Sajikan analisis dalam 3 poin utama. Tuliskan dalma bahasa yang singkat, padat, dan jelas.
...
def risk_analysis(financials: pd.DataFrame):
    ...
    {data}
    Identifikasi 2-3 potensi risiko atau area yang perlu diwaspadai
